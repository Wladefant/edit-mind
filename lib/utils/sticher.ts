...
export async function stitchVideos(
  ...
  if (resW && resH) {
    targetWidth = resW;
    targetHeight = resH;
    // Optional: You could add a check here to warn if targetResolution's aspect ratio
    // doesn't match the requested aspectRatio. For now, we'll trust the input.
  } else {
    ...
    if (arNum / arDen >= 1) {
      ...
      targetWidth = 1080; // or 720 for smaller portrait videos
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    await new Promise<void>((resolve, reject) => {
      const baseFfmpegArgs = [
        ...
        "-i",
        scene.source,
        // Video filters: scale, pad, set frame rate
        "-vf",
      ...
      const ffmpeg = spawn("ffmpeg", [
        ...baseFfmpegArgs.slice(0, 10), // Up to -c:v
        "-map", // Try to map video and audio streams
        "0:v:0",
        "-map",
        "0:a:0?", // Map audio stream if it exists, otherwise ignore
      ...
      ffmpeg.on("close", (code) => {
        if (code === 0) {
          ...
          const ffmpegRetry = spawn("ffmpeg", [
            ...
            "-map", "1:a:0", // Map the generated silent audio stream
          ...
          ffmpegRetry.stderr.on("data", (data) =>
            console.error(`ffmpeg retry stderr: ${data}`)
          );
